...
void InitMemoryList(CListCtrl *m_ListCtrl)
{
	...
	for (i = 0; i<g_Column_Memory_Count; i++)
	{
		if (i == 1)
		{
			m_ListCtrl->InsertColumn(i, g_Column_Memory[i].szTitle, LVCFMT_RIGHT, (int)(g_Column_Memory[i].nWidth));
		}
		else
		{
			m_ListCtrl->InsertColumn(i, g_Column_Memory[i].szTitle, LVCFMT_LEFT, (int)(g_Column_Memory[i].nWidth));
		}
...
BOOL SendIoControlCodeModule(ULONG_PTR ProcessID)
{
	...
	do
	{
		...
		bRet = DeviceIoControl(g_hDevice, IOCTL(IOCTL_PROC_PROCESSMODULE),
			&ProcessID,
			sizeof(ULONG_PTR),
			AllModules,
			ulSize,
...
BOOL SendIoControlCodeMemory(ULONG_PTR ProcessID)
{
	...
	do
	{
		...
		bRet = DeviceIoControl(g_hDevice, IOCTL(IOCTL_PROC_PROCESSMEMORY),
			&ProcessID,
			sizeof(ULONG_PTR),
			Memorys,
			ulSize,
