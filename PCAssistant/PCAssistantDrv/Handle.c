...
NTSTATUS EnumProcessHandle(PVOID InBuffer, ULONG_PTR InSize, PVOID OutBuffer, ULONG_PTR OutSize)
{
	...
	InitHandleVariable();
	NtQueryObjectAddress = (pfnNtQueryObject)GetFuncAddress("NtQueryObject");
	...
	{
		Status = GetHandles(ulPid, (ULONG_PTR)EProcess, (PALL_HANDLES)OutBuffer, ulCount);
	}
			...
			if (NT_SUCCESS(Status))
			{
				...
				for (i = 0; i < Handles->NumberOfHandles; i++)
				{
					if (ulPid == Handles->Handles[i].UniqueProcessId)
					{
						...
						{
							InsertHandleToList((PEPROCESS)EProcess,
...
VOID InsertHandleToList(PEPROCESS EProcess, HANDLE Handle, ULONG_PTR Object, PALL_HANDLES Handles)
{
		...
		(Buffer = (PHANDLE_INFO)ExAllocatePool(NonPagedPool, sizeof(HANDLE_INFO))) != NULL)
	{
		...
		GetHandleObjectName((HANDLE)Handle, Buffer->HandleName);
		GetHandleTypeName((HANDLE)Handle, Object, Buffer->ObjectName);
